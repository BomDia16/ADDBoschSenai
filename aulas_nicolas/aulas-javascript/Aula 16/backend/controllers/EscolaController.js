...
export const getPeople = (req, res) => {
    const users = connection.query("select * from escola", (err, results) => {
        if (err) {
...
export const createUser = (req, res) => {
    const { descricao, local_encontrado, status } = req.body

    try {
        const inserir = connection.query(
            "insert into escola (descricao, local_encontrado, status) values (?,?,?);",
            [descricao, local_encontrado, status]
        )
